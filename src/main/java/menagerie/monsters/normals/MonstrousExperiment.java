...
public class MonstrousExperiment extends CustomMonster
{
    ...
    protected void getMove(final int num) {
        if ((this.moveHistory.size() >= 2 && !this.lastMove(DEGENERATE_MOVE) && !this.lastMoveBefore(DEGENERATE_MOVE) && (num > 50 || !this.lastMoveBeforeBefore(DEGENERATE_MOVE)))
            ...
            byte moveBeforeLastNonDegenerateMove = movesWithoutDegenerate.size() >= 2 ? movesWithoutDegenerate.get(movesWithoutDegenerate.size() - 2) : 0;

            if (!this.firstMove && (lastNonDegenerateMove == SLIMY_APPENDAGE_ATTACK || (moveBeforeLastNonDegenerateMove != THRASH_ATTACK && num < 65))){
